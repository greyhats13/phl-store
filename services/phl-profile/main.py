...
class Settings(BaseSettings):
    db_user: str = "profile"
    db_password: str = "passwdDev123"
    db_host: str = "mysql"
    db_port: int = 3306
    db_name: str = "profiles"
...
class ProfileService:
    ...
    async def update_profile(self, userid: int, profile_data: ProfileUpdateModel) -> Profile:
        profile = await self.session.get(Profile, userid)
        if not profile:
            raise HTTPException(status_code=404, detail="Profile not found")
...
async def create_profile(profile: ProfileCreateModel, session: AsyncSession = Depends(get_session)):
    service = ProfileService(session)
...
async def update_profile(userid: int, profile_data: ProfileUpdateModel, session: AsyncSession = Depends(get_session)):
    service = ProfileService(session)
